...
public class TestSelfRegistrationManager extends TestBase {
    ...
    public void testUnregistration() throws Exception {
        ...
        inOrder.verify(jobManager).hasOngoingJobs();
    ...
    /**
     * Test that ongoing jobs are outwaited before service deregistration
     */
    @Test
    public void testJobOutwait() throws Exception {
        //given
        Subscription subscription = new Subscription();
        subscription.setCallbackUrl("http://5.6.7.8:12345/api/NokiaSVNFM/v1/lcn");
        subscription.setId(UUID.randomUUID().toString());
        subscriptions.add(subscription);
        when(jobManager.hasOngoingJobs()).thenAnswer(new Answer<Boolean>() {
            int count = 0;
        ...
        selfRegistrationManager.unregister();
        //verify
        verify(jobManager, Mockito.times(2)).hasOngoingJobs();
        verify(systemFunctions).sleep(500);
    }

    /**
     * if the MSB reports that it could not cancel the service, but the service has
     * disaperared from MSB the cancellation is considered to be successfull
     */
    @Test
    public void testPartialyFailedMsbCancel() throws Exception {
        //given
        Subscription subscription = new Subscription();
        subscription.setCallbackUrl("http://5.6.7.8:12345/api/NokiaSVNFM/v1/lcn");
        subscription.setId(UUID.randomUUID().toString());

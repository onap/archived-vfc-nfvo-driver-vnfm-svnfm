...
public class OnapR2HeatPackageBuilder {
    ...
    private void processVdus(JsonArray policies, StringBuilder envContent, StringBuilder yamlContent) {
        JsonObject heatPolicy = locateHeatPolicy(policies);
        JsonObject properties = child(heatPolicy, "properties");
        if (properties.has("static")) {
            JsonObject base = child(properties, "static");
            processMapping(base, envContent, yamlContent);
        }
    }

...
/**
 * Responsible for registering the driver in the external systems.
 */
@Component
public class SelfRegistrationManager {
    ...
    /**
     * Unregister the VNFM driver from the micro-service bus
     */
    ...
    /**
     * Wait for all jobs to be cleared from the system the refuses to let addintonal request in
     */
    private void prepareForShutdown() {
        while (true) {
            synchronized (jobManager) {
                if (!jobManager.hasOngoingJobs()) {
                    ready = false;
                    return;
                }
            }

...
public class SoLifecycleManager {
    ...
    @Autowired
    SoLifecycleManager(LifecycleManager lifecycleManager, AAIExternalSystemInfoProvider aaiExternalSystemInfoProvider, CbamRestApiProvider cbamRestApiProvider, JobManager jobManager) {
    ...
    public JobHandler activate(String vnfmId, String vnfId, VnfActivationRequest soRequest, HttpServletResponse httpResponse) {
        ...
        additionalParameters.setVimType(vimTypeHeuristic(vimInfo.getServiceUrl()));
     ...
     * @return the job handler of the VNF activation
     */
    public JobHandler customOperation(String vnfmId, String vnfId, VnfCustomOperation request, HttpServletResponse httpResponse) {
     ...
     */
    public JobDetail getJobDetails(String vnfmId, String jobId) {
        ...
        if(JobStatus.STARTED.equals(currentStatus)){
            jobDetail.setStatus(org.onap.vnfmadapter.so.model.JobStatus.STARTED);
        }
        else if(JobStatus.PROCESSING.equals(currentStatus)){
            jobDetail.setStatus(org.onap.vnfmadapter.so.model.JobStatus.STARTED);
        }
        else if(JobStatus.FINISHED.equals(currentStatus)){
            jobDetail.setStatus(org.onap.vnfmadapter.so.model.JobStatus.FINISHED);
        }
        else if(JobStatus.TIMEOUT.equals(currentStatus)){
            jobDetail.setStatus(org.onap.vnfmadapter.so.model.JobStatus.FAILED);
        }
        else {//ERROR
            jobDetail.setStatus(org.onap.vnfmadapter.so.model.JobStatus.FAILED);
        }
